...
import { NeuroBranchMark } from '../NeuroBranchMark'
...
export function AppHeader({ onOpenSearch, onOpenSettings, onWorkspaceChange, searchDisabled = false, searchLabel, searchShortcut, settingsOpen, workspace }: { onOpenSearch(): void; onOpenSettings(): void; onWorkspaceChange(workspace: StudioWorkspaceId): void; searchDisabled?: boolean; searchLabel: string; searchShortcut: string; settingsOpen: boolean; workspace: StudioWorkspaceId }) {
  return <header className="topbar">
    <div className="brand"><span className="brand-mark"><NeuroBranchMark /></span><strong>NeuroBranch</strong><span className="alpha-pill">BETA</span></div>
    <nav aria-label="NeuroBranch studios" className="studio-navigation">{studios.map((studio) => <button aria-pressed={workspace === studio.id} key={studio.id} onClick={() => onWorkspaceChange(studio.id)} type="button">{studio.label}</button>)}</nav>
    <div className="header-actions">
      ...
      <StudioButton aria-label="Open NeuroBranch settings" aria-pressed={settingsOpen} className="codex-button" onClick={onOpenSettings} variant="accent"><Settings2 size={14} />Settings</StudioButton>
    </div>
  </header>
}
