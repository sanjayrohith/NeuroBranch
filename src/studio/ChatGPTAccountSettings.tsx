...
export function ChatGPTAccountSettings({ busy, session, onConfigurationChange, onConnect, onDisconnect }: {
  ...
  return <section className="ask-neurobranch-key-settings ask-neurobranch-chatgpt-settings">
    <div className="ask-neurobranch-key-heading"><span><UserRound size={13} />ChatGPT account</span>{session?.connected && <b><span className="status-dot" />Connected</b>}</div>
    {session?.connected ? <>
      <div className="ask-neurobranch-key-status"><ShieldCheck size={16} /><span><strong>{session.email || 'ChatGPT account connected'}</strong><small>{session.planType ? `${session.planType} plan · ` : ''}Ask NeuroBranch uses this session by default. No API key is required.</small></span></div>
      ...
      <div className="ask-neurobranch-key-actions"><button disabled={busy} onClick={onDisconnect} type="button"><LogOut size={12} />Disconnect ChatGPT</button></div>
    </> : <div className="ask-neurobranch-key-form">
