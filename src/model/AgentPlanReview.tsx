...
import type { NeuroBranchLanguage } from '../studio/application-language'
import { agentPlanReviewText } from './agent-plan-review-text'

interface AgentPlanReviewProps {
  language: NeuroBranchLanguage
  plan: AgentGraphPlan
...
export function AgentPlanReview({ language, onApply, onDiscard, onEditCard, plan, preview }: AgentPlanReviewProps) {
  ...
  return <section aria-label={copy.title} className="agent-plan-review">
    <div className="ask-neurobranch-result agent-plan-review-content">
      <section><h3>{copy.plan}</h3><p>{plan.summary}</p></section>

      {(plan.toolTrace?.length ?? 0) > 0 && <section>
        <h3>{copy.tools}</h3>
        <ul className="ask-neurobranch-tool-trace">{plan.toolTrace!.map((item, index) => <li data-status={item.status} key={`${item.tool}-${index}`}><code>{item.tool}</code><span>{item.summary}</span></li>)}</ul>
      </section>}
      ...
      {preview.acceptedBlocks.length > 0 && <section>
        ...
        <div className="ask-neurobranch-added-blocks">{preview.acceptedBlocks.map((block) => {
      ...
      {preview.acceptedCreatedBlocks.length > 0 && <section>
        ...
        <div className="ask-neurobranch-created-blocks">{preview.acceptedCreatedBlocks.map((block) => {
          const node = preview.graph.nodes.find((candidate) => candidate.id === block.nodeId)
      ...
      {plan.missingBlocks.length > 0 && <section className="ask-neurobranch-missing"><h3>{copy.missing}</h3>{plan.missingBlocks.map((block, index) => <div key={`${block.atomId ?? block.label}-${index}`}><AlertTriangle size={13} /><span><strong>{block.label}</strong><small>{block.reason}</small></span></div>)}</section>}

      {(preview.rejectedBlocks.length > 0 || preview.rejected.length > 0 || preview.rejectedMutations.length > 0 || plan.warnings.length > 0) && <section className="ask-neurobranch-warnings">
        <h3>{copy.rejected}</h3>
    ...
    <footer className="ask-neurobranch-actions agent-plan-review-actions">
      <span className="ask-neurobranch-approval-summary">{copy.ready} · {preview.acceptedBlocks.length + preview.acceptedCreatedBlocks.length} {copy.cards} · {preview.accepted.length} {copy.elastic}{preview.accepted.length === 1 ? '' : 's'}</span>
      <button className="ask-neurobranch-cancel" onClick={onDiscard} type="button">{copy.discard}</button>
      <button aria-label="Apply full graph plan" className="ask-neurobranch-apply" disabled={!canApply} onClick={onApply} type="button"><Check size={13} />{copy.apply}</button>
    </footer>
  </section>
}
